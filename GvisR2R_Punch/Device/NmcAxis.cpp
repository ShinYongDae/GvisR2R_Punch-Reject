...
int CNmcAxis::StartRsaHoming(BOOL bWait, int nMode, int nDir)
{
	...
	else
	{
		...
		if (ms != MC_OK) { pView->SetAlarmToPlc(UNIT_PUNCH); pView->ClrDispMsg(); AfxMessageBox(_T("Error-StartRsaHoming(01)")); return ms; }
		ms = MC_WriteIntParameter(m_nBoardId, vAxisID, mcpHomingDir, (UINT32)nDir);
		if (ms != MC_OK) { pView->SetAlarmToPlc(UNIT_PUNCH); pView->ClrDispMsg(); AfxMessageBox(_T("Error-StartRsaHoming(02)")); return ms; }
		ms = MC_WriteParameter(m_nBoardId, vAxisID, mcpHomingVelocity, dVel);				// 1st Speed
		if (ms != MC_OK) { pView->SetAlarmToPlc(UNIT_PUNCH); pView->ClrDispMsg(); AfxMessageBox(_T("Error-StartRsaHoming(03)")); return ms; }
		ms = MC_WriteParameter(m_nBoardId, vAxisID, mcpHomingAcceleration, dAcc);
		if (ms != MC_OK) { pView->SetAlarmToPlc(UNIT_PUNCH); pView->ClrDispMsg(); AfxMessageBox(_T("Error-StartRsaHoming(04)")); return ms; }
		ms = MC_WriteParameter(m_nBoardId, vAxisID, mcpHomingDeceleration, dAcc);
		if (ms != MC_OK) { pView->SetAlarmToPlc(UNIT_PUNCH); pView->ClrDispMsg(); AfxMessageBox(_T("Error-StartRsaHoming(05)")); return ms; }
		ms = MC_WriteParameter(m_nBoardId, vAxisID, mcpHomingJerk, dJerk);
		if (ms != MC_OK) { pView->SetAlarmToPlc(UNIT_PUNCH); pView->ClrDispMsg(); AfxMessageBox(_T("Error-StartRsaHoming(06)")); return ms; }
		ms = MC_WriteParameter(m_nBoardId, vAxisID, mcpHomingCreepVelocity, dVel2);			// 2nd Speed
		if (ms != MC_OK) { pView->SetAlarmToPlc(UNIT_PUNCH); pView->ClrDispMsg(); AfxMessageBox(_T("Error-StartRsaHoming(07)")); return ms; }
		ms = MC_WriteParameter(m_nBoardId, vAxisID, mcpHomePositionOffset, dOffset);
		if (ms != MC_OK) { pView->SetAlarmToPlc(UNIT_PUNCH); pView->ClrDispMsg(); AfxMessageBox(_T("Error-StartRsaHoming(08)")); return ms; }
		ms = MC_WriteIntParameter(m_nBoardId, vAxisID, mcpHomeCompleteFlagHandle, HOMING_CLEAR);
		if (ms != MC_OK) { pView->SetAlarmToPlc(UNIT_PUNCH); pView->ClrDispMsg(); AfxMessageBox(_T("Error-StartRsaHoming(09)")); return ms; }
	...
	if (bWait)
	{
		...
		if (ms != MC_OK) { pView->SetAlarmToPlc(UNIT_PUNCH); pView->ClrDispMsg(); AfxMessageBox(_T("Error-StartRsaHoming(11)")); return ms; }
		...
		while (!(state & mcHoming))
		{
			...
			Sleep(100);
			ms = MC_ReadAxisStatus(m_nBoardId, vAxisID, &state);
			if (ms != MC_OK) { pView->SetAlarmToPlc(UNIT_PUNCH); pView->ClrDispMsg(); AfxMessageBox(_T("Error-StartRsaHoming(12)")); return ms; }
		}

		ms = MC_ReadAxisStatus(m_nBoardId, vAxisID, &state);
		if (ms != MC_OK) { pView->SetAlarmToPlc(UNIT_PUNCH); pView->ClrDispMsg(); AfxMessageBox(_T("Error-StartRsaHoming(13)")); return ms; }

		while (!(state & mcIsHomed))
		{
			if (GetTickCount() - nTick >= 600000) { pView->SetAlarmToPlc(UNIT_PUNCH); pView->ClrDispMsg(); AfxMessageBox(_T("Error-StartRsaHoming(14)")); return ms; }
			Sleep(100);
			ms = MC_ReadAxisStatus(m_nBoardId, vAxisID, &state);
			if (ms != MC_OK) { pView->SetAlarmToPlc(UNIT_PUNCH); pView->ClrDispMsg(); AfxMessageBox(_T("Error-StartRsaHoming(15)")); return ms; }
		}
...
UINT CNmcAxis::RsaHomeThreadProc(LPVOID lpContext)
{
	...
	if (pThread->IsGantry())
	{
		...
		if (ms != MC_OK)
		{
			...
			pView->SetAlarmToPlc(UNIT_PUNCH); pView->ClrDispMsg(); AfxMessageBox(_T("Error-StartRsaHoming(200)"));
			return 0;
		}
		...
		while (!(state & mcGantry_Homing) && pThread->m_bRsaHomeThreadAlive)
		{
			if (GetTickCount() - nTick >= 3000)
			{
				pThread->m_bHomeThreadAlive = FALSE;
				pThread->m_bRsaHomeThreadAlive = FALSE;
				pView->SetAlarmToPlc(UNIT_PUNCH); pView->ClrDispMsg(); AfxMessageBox(_T("Error-StartRsaHoming(201)"));
				return 0;
			}
		...
		if (ms != MC_OK)
		{
			pThread->m_bHomeThreadAlive = FALSE;
			pThread->m_bRsaHomeThreadAlive = FALSE;
			pView->SetAlarmToPlc(UNIT_PUNCH); pView->ClrDispMsg(); AfxMessageBox(_T("Error-StartRsaHoming(203)"));
			return 0;
		}
		...
		while (!(state & mcGantry_IsHomed) && pThread->m_bRsaHomeThreadAlive)
		{
			if (GetTickCount() - nTick >= 600000)
			{
				pThread->m_bHomeThreadAlive = FALSE;
				pThread->m_bRsaHomeThreadAlive = FALSE;
				pView->SetAlarmToPlc(UNIT_PUNCH); pView->ClrDispMsg(); AfxMessageBox(_T("Error-StartRsaHoming(204)"));
				return 0;
			}
			Sleep(pThread->m_nSamplingTimeMsec);
			...
			if (ms != MC_OK)
			{
				pThread->m_bHomeThreadAlive = FALSE;
				pThread->m_bRsaHomeThreadAlive = FALSE;
				pView->SetAlarmToPlc(UNIT_PUNCH); pView->ClrDispMsg(); AfxMessageBox(_T("Error-StartRsaHoming(205)"));
				return 0;
			}
		}
	}
	else
	{
		...
		if (ms != MC_OK)
		{
			pThread->m_bHomeThreadAlive = FALSE;
			pThread->m_bRsaHomeThreadAlive = FALSE;
			pView->SetAlarmToPlc(UNIT_PUNCH); pView->ClrDispMsg(); AfxMessageBox(_T("Error-StartRsaHoming(100)"));
			return 0;
		}
		...
		while (!(state & mcHoming) && pThread->m_bRsaHomeThreadAlive)
		{
			if (GetTickCount() - nTick >= 3000)
			{
				pThread->m_bHomeThreadAlive = FALSE;
				pThread->m_bRsaHomeThreadAlive = FALSE;
				pView->SetAlarmToPlc(UNIT_PUNCH); pView->ClrDispMsg(); AfxMessageBox(_T("Error-StartRsaHoming(101)"));
				return 0;
			}
			Sleep(pThread->m_nSamplingTimeMsec);
			...
			if (ms != MC_OK)
			{
				pThread->m_bHomeThreadAlive = FALSE;
				pThread->m_bRsaHomeThreadAlive = FALSE;
				pView->SetAlarmToPlc(UNIT_PUNCH); pView->ClrDispMsg(); AfxMessageBox(_T("Error-StartRsaHoming(102)"));
				return 0;
			}
		}
		...
		if (ms != MC_OK)
		{
			pThread->m_bHomeThreadAlive = FALSE;
			pThread->m_bRsaHomeThreadAlive = FALSE;
			pView->SetAlarmToPlc(UNIT_PUNCH); pView->ClrDispMsg(); AfxMessageBox(_T("Error-StartRsaHoming(103)"));
			return 0;
		}
		...
		while (!(state & mcIsHomed) && pThread->m_bRsaHomeThreadAlive)
		{
			if (GetTickCount() - nTick >= 600000)
			{
				pThread->m_bHomeThreadAlive = FALSE;
				pThread->m_bRsaHomeThreadAlive = FALSE;
				pView->SetAlarmToPlc(UNIT_PUNCH); pView->ClrDispMsg(); AfxMessageBox(_T("Error-StartRsaHoming(104)"));
				return 0;
			}
			Sleep(pThread->m_nSamplingTimeMsec);
			...
			if (ms != MC_OK)
			{
				pThread->m_bHomeThreadAlive = FALSE;
				pThread->m_bRsaHomeThreadAlive = FALSE;
				pView->SetAlarmToPlc(UNIT_PUNCH); pView->ClrDispMsg(); AfxMessageBox(_T("Error-StartRsaHoming(105)"));
				return 0;
			}
		}
	}
